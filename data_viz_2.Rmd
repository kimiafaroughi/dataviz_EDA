...
```{r}
weather_df |> 
  mutate(name = fct_relevel(name, c("Molokai_HI", "CentralPark_NY", "Waterhole_WA"))) |> #reorders based on factor var so ggplot knows where they go on the x axis
...
```{r}
weather_df |> 
  mutate(name = fct_reorder(name, tmax)) |> #in order of avg tmax per name group from smallest to largest
  ggplot(aes(x = name, y = tmax, fill = name)) +
  geom_violin(alpha = 0.5)
```
